...
Owner Aug 1, 2026 — self-serve Library maintenance "for when you won't always be available", so rename/overwrite/delete don't require an agent.
...
**Smoke:** `library-admin.html` with admin mode off → gate screen only, no table. Enable → table lists current Library entries plus Publish/overwrite, **Free a live :8080 name**, category board, **Test Drive doors**, and any leftover hidden catalog ids. Delete a listing → gone from `library.html`; a later Publish of the same name can reuse the slug. Delete a live uniqueId row → `:3001` must be up; occupancy no longer blocks that **name**. Review: `http://localhost:5500/library-admin.html?v=20260909-tddoor1&admin=1`.
