...
**PUBLISH** (wired Aug 1, 2026 — was grey stub; live-form gate Aug 26; purge-on-Publish Aug 26; duplicate-product refuse Aug 26): confirm → opens a dialog (name editable, default = current name with any `" (stub)"` suffix stripped, since a Publish target is never itself a stub) → `TawalaTransfer.publishToLibrary` **aborts with no overlay write** if the source My Tawala project has no live form (no uniqueId and none resolvable from start URLs / `testDriveUrl` — same as Purge). Owner copy: *This project has no live form yet. Edit in Designer, Push, then Publish.* (no uniqueId in the alert). Details dialog may disable **Publish** for no live form; library-admin can still click — both fail closed in `publishToLibrary`. **Duplicate Library product** (owner Aug 26): refuse before any purge if the project is otherwise identical to a visible Library listing (`pulledFromLibraryId` / fork chain, same `jsonFile`, or compact-equal Publish name) unless `replaceLibraryId` is that listing **and** the current mock user is its listed `author`. Catalog seeds have no author — Copy of OEB / House Test empty OEB cannot add a new row or overwrite catalog OEB. Overlay stamps `author` on successful Publish. Refuse copy: *This is the same as “Online Exam Builder” already in the Library. You can only publish an update if you are that listing’s author.* **Cannot Publish with data** (owner Aug 26): if Records count > 0 (same uniqueId as Purge), extra confirm *Project will be purged of data upon publication. Proceed?* — Cancel = no overlay and no purge. Proceed = whole-uniqueId purge, **then** Library overlay write. Empty templates skip the extra confirm. There is **no** “keep responses” checkbox. See § Publish below.
...
**Smoke Publish:** open http://localhost:5500/mytawala-project.html?v=20260826-pubdup1 (or Library admin `?v=20260826-pubdup1&admin=1`). **Refuse live form:** Publish the leftover **Horses** (or any row with no live form / no `/p/{id}/` starts) → Details dialog disables Publish (or admin click fails) with *This project has no live form yet. Edit in Designer, Push, then Publish.* — no new Library overlay. **Refuse duplicate:** Copy to MyTawala **Online Exam Builder** (or Get Together), optionally rename House Test, Publish with replace = None → *This is the same as “Online Exam Builder” already in the Library. You can only publish an update if you are that listing’s author.* — no purge confirm, no overlay. Catalog OEB has no author — picking it as overwrite also refuses. **Author update:** after a successful original Publish (overlay `author` stamped), Publish again with that overlay selected as replace → allowed (then purge confirm if Records > 0). **File→New-like add:** a Push row with no `pulledFromLibraryId` / `jsonFile` twin and a unique name can add a new listing. **Still works:** Publish a Designer-**Push** original that has a live form → overlay writes. Then: open a live original → Project Details → **PUBLISH** → dialog opens with the name pre-filled (no keep-data checkbox) → type a new name, e.g. “DirtBowl League Manager”, pick the flagged “DirtBowl (stub) — retires to My Tawala, kept marked (stub)” target → **Publish** → confirm → if Records > 0, extra confirm *Project will be purged of data upon publication. Proceed?* (Cancel leaves the project unpublished and unpurged) → alert confirms the new Library entry, the retired stub, and a purged-row count when data was purged → reload `library.html?v=20260826-pubdup1`: new name appears (no `(stub)` suffix), old “DirtBowl (stub)” row is gone → reload `mytawala.html?v=20260826-pubdup1`: a new private row named “DirtBowl (stub)” appears (safety copy). Repeat picking **None** to just add a new entry with no retirement, and picking a **non-stub** target (e.g. an out-of-date Main Menu template) to see it replaced in place with no My Tawala copy. Empty templates skip the purge confirm. Pick a project with a live `:8080` deploy and prior submissions, Publish it (accept purge), then **Copy to MyTawala** the newly published Library entry — Records on the acquire should start empty.
...
**Scratch (Aug 26):** same-uniqueId partial purge (keep Question/SetupVariables) **reverted** — it scrambled Online Exam. Data-driven Library entries have **no Test Drive**. **Copy to MyTawala** = empty clone (does not import catalog submissions). **Make a Copy** (own fork) copies **all** response data onto the new uniqueId. **Publish** with Records > 0: confirm *Project will be purged of data upon publication. Proceed?* then whole-uniqueId purge, then overlay. Empty templates skip the extra confirm. Duplicate Library product refused unless the listed author is updating that overlay (`pubdup1`). Review `library.html?v=20260826-pubdup1`.
...
3. **Confirm** — a `window.confirm()` spelling out the consequence (new entry / stub retirement / in-place replace). Duplicate-product refuse (if any) happens next, **before** purge. If Records > 0, a **second** confirm is the exact purge string above.
4. **Purge then overlay** — when there is data and they proceed, purge first; only then write `tawala.mock.libraryOverlay`.

**Purge-on-Publish (updated Aug 26, 2026):** duplicate-product refuse runs first (no wipe). Purge runs **before** the overlay write when count > 0 (after the purge confirm). Same uniqueId resolution as My Tawala **PURGE** (`resolvePurgeUniqueId`), same `POST :3001/api/purge-responses` call. If purge fails, Publish **aborts** (no Library overlay). Empty templates skip purge. Does **not** run on **Retire**, **Rename**, or a category move.
