...
- **My Tawala is not a fake catalog.** It shows the user’s real projects only: Designer **Push / Show in My Tawala**, and Library **Save a copy** (including **Get from Library…** on the My Tawala bar). `TAWALA_MYTAWALA` seed is **empty** (Aug 10, 2026) — converted-backup / archive stubs and the fake Online Exam My Tawala seed were removed. JSON under `projects/mytawala/` may remain on disk; do not re-seed undeployed stubs into the listing. Listing blue **from Library** chip shows only on the acquire’s **local calendar day** (same date as `createdAt` / `created`); Details sidebar Source stays forever. Hard-refresh: `http://127.0.0.1:5500/mytawala.html?v=20260810-usestarts1`
- **Always use `http://127.0.0.1:5500`** (not `localhost`) — `serve.sh` binds 127.0.0.1, and browsers treat `localhost` vs `127.0.0.1` as **different localStorage**. Designer **Show in My Tawala** now opens 127.0.0.1 so Push merges into the same overlay as Library Save a copy. If acquires “vanished” after Push, check My Tawala on the other host or DevTools → Application → Local Storage for both origins (`tawala.mock.myTawalaOverlay`).
...
| Verb | Scope | Meaning |
|------|--------|---------|
| **Save a copy** (Library) | Library → My Tawala | Copy a public Library project into *this account’s* private My Tawala (acquire). Invites a name on the way in (suggested Library title or `Copy of …`); **warn + confirm overwrite** if the name matches an existing My Tawala row (case-insensitive trim) — never silent. **Owner priority (Aug 10):** Use must work when the copy lands — mock copies Library live start URLs / uniqueId (`mockSharedLibraryRuntime`; demo limitation — production mints a private uniqueId). Stays private until Publish. Rename anytime later on Project Details. Not open in Designer. Wired Aug 9 #8; naming + Use-ready Aug 10 (`TawalaTransfer.saveCopyFromLibrary`). See § Acquire naming. |
...
http://127.0.0.1:5500/mytawala-project.html?project=online-exam-builder&v=20260809-demorec1  
...
**Smoke Delete:** open http://127.0.0.1:5500/mytawala.html → select a project radio → **DELETE** → confirm → row gone; reload → still gone; Library twin unchanged. Deploy overlay: **Show in My Tawala**, select row → Delete → overlay + inbox entry gone; Library catalog unchanged.
...
**Smoke Publish:** open http://127.0.0.1:5500/mytawala.html → open a project → Project Details → **PUBLISH** → dialog opens with the name pre-filled and **"Purge responses when publishing"** checked by default → type a new name, e.g. “DirtBowl League Manager”, pick the flagged “DirtBowl (stub) — retires to My Tawala, kept marked (stub)” target → **Publish** → confirm → alert confirms the new Library entry, the retired stub, **and** either a purged-row count or a clear ⚠️ note if purge was skipped/failed → reload `library.html`: new name appears (no `(stub)` suffix), old “DirtBowl (stub)” row is gone → reload `mytawala.html`: a new private row named “DirtBowl (stub)” appears (safety copy). Repeat picking **None** to just add a new entry with no retirement, and picking a **non-stub** target (e.g. an out-of-date Main Menu template) to see it replaced in place with no My Tawala copy. Uncheck the purge checkbox once to confirm Publish still succeeds and the alert says responses were **not** purged. Pick a project with a live `:8080` deploy and prior test-drive submissions, Publish it with the checkbox checked, then **Test drive** the newly published Library entry — it should open with a clean slate, not the previous account's answers.
...
**Smoke Get / Refresh:** open http://127.0.0.1:5500/mytawala.html?v=20260810-usestarts1 → **GET FROM LIBRARY…** (no selection needed) → picker lists Library projects → **Continue…** → Save a copy rename → lands on **My Tawala listing** with the new row highlighted, blue **from Library** chip (same calendar day only), and **Use** available on that row. Select a My Tawala row that is Library-linked → **REFRESH FROM LIBRARY** enables → dialog → **Refresh** → confirm → description/category refreshed; **name** / deploy identity unchanged. Select a private-only (Push) row with no Library link → Refresh stays disabled with a “not linked” tooltip; **DELETE** still enables.
...
1. **Lean the My Tawala listing** — **DONE (Aug 9 slice):** listing is Name · Created · Updated · Records · Use + quiet Details cue. Top bar: Get / Refresh / Delete (equal-height buttons). Heavy ops on Project Details only. **Next:** item 2 (Details ops rail grouping) — do not re-add the icon strip.
...
8. **Library listing / detail acquire clarity** — **DONE (Aug 9 #8; Use-ready Aug 10):** Listing + detail show blurb under the name; CTAs = **Test Drive** + **Save a copy** only. **Save a copy** → rename dialog → My Tawala overlay (`pulledFromLibraryId`); **Use works** via Library live start metadata in the mock (`mockSharedLibraryRuntime` — production must mint private uniqueId). Clone count bumps on acquire (Times used / clone display still Task #13). Test Drive leave/wipe honesty remains Task #14.
...
**Aug 10 review (empty My Tawala + Get from Library acquire + runnable Save a copy — no commit until owner asks):**

- My Tawala (expect empty or only Push / prior Save a copy): `http://127.0.0.1:5500/mytawala.html?v=20260810-usestarts1`
- **Use → starts open:** multi-start row (Online Exam or runnable Save a copy with 2+ starts) → listing **Use** → Details Project Data shows starts + first start selected; banner **Use** / **Copy link** enabled. Double-click same row → Details tree stays collapsed.
- **Get from Library…** → picker → Save a copy → listing shows **from Library** chip today; open Details → sidebar Source **from … in Library** stays. Chip expiry: next local calendar day (or console — set overlay `createdAt` to yesterday ISO, reload listing — chip gone, Details Source remains).
- Library browse / Test Drive (top nav): `http://127.0.0.1:5500/library.html?v=20260810-acqlst1`
