...
- **Rename ≠ Make a Copy.** **Rename** only changes the display name of the **same** project (Details + listing). **Make a Copy** mints a **new** My Tawala identity (new overlay id + name dialog with overwrite warn; original untouched) and **Pushes** a **new** `:8080` uniqueId (never share the source’s live id), then copies **all** source submissions onto that id (owner **Purges** for a clean slate). **Use / Deploy / Publish** work without opening Designer. Fail closed if `:3001`/`:8080` cannot clone (no uniqueId-null leftover row). Fail-open if data copy fails (clone stays; alert). Controls on Project Details (next to Rename) **and** My Tawala listing bar when a row is selected. ≠ Library **Copy to MyTawala** (empty clone from public catalog — does **not** copy catalog submissions).
...
- Glossary: **Use** (My Tawala — single-start run link, or Project Details with starts expanded + first start selected when multi-entry) ≠ **Copy to MyTawala** (Library → My Tawala; empty private clone; requires login) ≠ **Make a Copy** (own fork; copies **all** data) ≠ **open in Designer** ≠ legacy **USE IT** / CloneAndCustomize (not wired).
...
| **Library → My Tawala** | Listing / detail **Copy to MyTawala** | Wired (Aug 9 #8; private uniqueId Aug 24 #26) — rename dialog → Push catalog JSON as a **new** Tomcat identity (**empty** clone; does **not** import catalog/author submissions) → My Tawala overlay (`pulledFromLibraryId`, own uniqueId). Edit in Designer uses the clone snapshot. |
...
| Verb | Scope | Meaning |
|------|--------|---------|
| **Copy to MyTawala** (Library; was Save a copy / Save to MyTawala) | Library → My Tawala | Copy a public Library **project** into *this account’s* private My Tawala (acquire). **Hidden for guests**. Invites a name on the way in; **warn + confirm overwrite** if the name matches. **Aug 24 Task #26:** Pushes a **private** `:8080` clone (new uniqueId). **Aug 26:** empty clone only — does **not** import submissions from the catalog/author uniqueId (that is shared / other people’s rows). Edit in Designer opens that clone’s snapshot, not the catalog template. Stays private until Publish. ≠ **Make a Copy** (own fork, copies **all** data). Wired Aug 9 #8; naming Aug 10; label Aug 24; private uniqueId Aug 24 (`TawalaTransfer.saveCopyFromLibrary`). See § Acquire naming. |
| **Make a Copy** (My Tawala) | Own project fork | Fork a private My Tawala project into a **new** identity (Details + listing bar). Name dialog + overwrite warn; original untouched; **new** Tomcat uniqueId; then copies **all** source submissions onto that uniqueId (owner **Purges** for a clean slate). Clone kept if `:3001` export/import fails — alert that data did not copy. **Use / Deploy / Publish** work without opening Designer. **Version:** fresh line at **1** (*Copy of …*); does **not** copy source `versions[]`. Fail closed if clone Push fails. ≠ **Rename** (same id) ≠ Library **Copy to MyTawala**. Wired Aug 10 #9; clone-on-copy Aug 26 (`TawalaTransfer.makeCopyOfMyTawalaProject`). |
...
**PUBLISH** (wired Aug 1, 2026 — was grey stub; live-form gate Aug 26; empty Library clone Aug 26; duplicate-product refuse Aug 26): confirm → opens a dialog (name editable, default = current name with any `" (stub)"` suffix stripped, since a Publish target is never itself a stub) → `TawalaTransfer.publishToLibrary` **aborts with no overlay write** if the source My Tawala project has no live form (no uniqueId and none resolvable from start URLs / `testDriveUrl` — same as Purge). Owner copy: *This project has no live form yet. Edit in Designer, Push, then Publish.* (no uniqueId in the alert). Details dialog may disable **Publish** for no live form; library-admin can still click — both fail closed in `publishToLibrary`. **Duplicate Library product** (owner Aug 26): refuse before the empty Library clone if the project is otherwise identical to a visible Library listing (`pulledFromLibraryId` / fork chain, same `jsonFile`, or compact-equal Publish name) unless `replaceLibraryId` is that listing **and** the current mock user is its listed `author`. Catalog seeds have no author — Copy of OEB / House Test empty OEB cannot add a new row or overwrite catalog OEB. Overlay stamps `author` on successful Publish. Refuse copy: *This is the same as “Online Exam Builder” already in the Library. You can only publish an update if you are that listing’s author.* **Library copy is empty; My Tawala data stays** (owner Aug 26, corrected): Publish mints (or reuses) a **separate** Library uniqueId with no submissions. It does **not** purge the author’s My Tawala uniqueId. Confirm: *The Library copy will not include saved responses. Your My Tawala project is unchanged.* There is **no** “keep responses” checkbox. See § Publish below.
...
**Smoke Publish:** open http://localhost:5500/mytawala-project.html?v=20260826-pubpriv1 (or Library admin `?v=20260826-pubpriv1&admin=1`). **Refuse live form:** Publish the leftover **Horses** (or any row with no live form / no `/p/{id}/` starts) → Details dialog disables Publish (or admin click fails) with *This project has no live form yet. Edit in Designer, Push, then Publish.* — no new Library overlay. **Refuse duplicate:** Copy to MyTawala **Online Exam Builder** (or Get Together), optionally rename House Test, Publish with replace = None → *This is the same as “Online Exam Builder” already in the Library. You can only publish an update if you are that listing’s author.* — no overlay. Catalog OEB has no author — picking it as overwrite also refuses. **Author update:** after a successful original Publish (overlay `author` stamped), Publish again with that overlay selected as replace → allowed. **File→New-like add:** a Push row with no `pulledFromLibraryId` / `jsonFile` twin and a unique name can add a new listing. **Private data stays:** Publish a Designer-**Push** original that has saved responses → confirm *The Library copy will not include saved responses. Your My Tawala project is unchanged.* → Library Test Drive / Copy to MyTawala starts empty; My Tawala **Records** are still there. Then: open a live original → Project Details → **PUBLISH** → dialog opens with the name pre-filled (no keep-data checkbox) → type a new name, e.g. “DirtBowl League Manager”, pick the flagged “DirtBowl (stub) — retires to My Tawala, kept marked (stub)” target → **Publish** → confirm → alert confirms the new Library entry and the retired stub → reload `library.html?v=20260826-pubpriv1`: new name appears (no `(stub)` suffix), old “DirtBowl (stub)” row is gone → reload `mytawala.html?v=20260826-pubpriv1`: a new private row named “DirtBowl (stub)” appears (safety copy). Repeat picking **None** to just add a new entry with no retirement, and picking a **non-stub** target (e.g. an out-of-date Main Menu template) to see it replaced in place with no My Tawala copy.
...
Library / home **primary Test Drive** (listing icon + detail button) purges that project’s responses **before** opening `:8080`, so each fresh drive starts clean. That is appropriate for simple surveys with no setup state (Simple Survey / Horses / Get Together — full uniqueId purge-on-start).

**Scratch (Aug 26):** same-uniqueId partial purge (keep Question/SetupVariables) **reverted** — it scrambled Online Exam. Data-driven Library entries have **no Test Drive**. **Copy to MyTawala** = empty clone (does not import catalog submissions). **Make a Copy** (own fork) copies **all** response data onto the new uniqueId. **Publish** mints a separate empty Library uniqueId — does **not** purge the author’s My Tawala data (`pubpriv1`). Duplicate Library product refused unless the listed author is updating that overlay (`pubdup1`). **Retire hide (`retire1`):** public listing skips overlay when `libraryRetired[id]` is set (House Test stayed listed after admin Retire); a second Retire is idempotent hide (clears leftover overlay, no “Already retired.”); successful Publish `clearLibraryRetired` so that slug can list again. Review `library.html?v=20260826-pubpriv1`.

**Library Project Details start-point links do not purge on click** (same as My Tawala Details). Data-driven catalog rows do not open `:8080` Test Drive at all.
...
**Live form required (owner Aug 26, 2026):** `publishToLibrary` **aborts** (no Library overlay write) if the source My Tawala project has no live form — no uniqueId and none resolvable from `/p/{id}/` on `testDriveUrl` / startPoints (same resolution as Purge). Owner alert: *This project has no live form yet. Edit in Designer, Push, then Publish.* A row that has start URLs but a missing overlay uniqueId still publishes. Details dialog may disable **Publish**; admin Publish/overwrite can still click — both fail closed in `publishToLibrary` so admin cannot skip it.

**Cannot Publish the author’s data into Library (owner Aug 26, 2026, corrected):** Library listing gets a **separate empty** live form (new uniqueId, or the listing’s existing Library uniqueId on author overwrite). The author’s My Tawala uniqueId is **never purged** and submissions are **never copied** onto the Library copy. Fail closed if the clone fails or would reuse the author’s uniqueId — no overlay write. Confirm copy: *The Library copy will not include saved responses. Your My Tawala project is unchanged.* There is **no** “keep responses” checkbox and **no** extra purge-your-project confirm. If the listing already pointed at the author’s uniqueId (the old shared-id path), the next author Publish mints a new Library uniqueId and leaves My Tawala alone.

**Duplicate Library product (owner Aug 26, 2026):** if the project is **otherwise identical** to an existing visible Library project, **reject** publication unless the person is **updating a Library product of which they are the listed author**. Identical if any of: `pulledFromLibraryId` (or `forkedFromId` → that acquire) equals a visible Library id; `jsonFile` matches a visible listing’s template file; proposed Publish **name** compact-matches an existing Library **name**. Allowed only when the dialog’s `replaceLibraryId` is that existing id **and** the current mock user (`TawalaChrome.currentUser` / `data-tawala-user` / `"dev"`) matches `entry.author`. Catalog seed rows typically have **no** `author` — cannot Publish a Copy of OEB / Copy of Get Together / House Test empty OEB as a new Library row, and cannot overwrite catalog OEB. Successful Publish stamps `author: currentUser` on the overlay when missing; that author may Publish again with that overlay selected as replace. Refuse copy: *This is the same as “Online Exam Builder” already in the Library. You can only publish an update if you are that listing’s author.* Duplicate check runs **before** the empty Library clone. A File→New-like Push row with no lineage / jsonFile twin and a unique name can add a new listing. Library acquire stays an empty clone; Make a Copy still copies all data.
...
3. **Confirm** — a `window.confirm()` spelling out the consequence (new entry / stub retirement / in-place replace) plus *The Library copy will not include saved responses. Your My Tawala project is unchanged.* Duplicate-product refuse (if any) happens next, **before** the empty Library clone.
4. **Empty Library clone then overlay** — mint (or reuse) a separate Library uniqueId with no submissions, then write `tawala.mock.libraryOverlay` pointing at **that** uniqueId. The author’s My Tawala uniqueId is not purged.

**Empty Library uniqueId on Publish (updated Aug 26, 2026):** duplicate-product refuse runs first. Then `emptyLibraryRuntimeForPublish` clones the definition under `_freshFromTemplate` (or redeploys onto the listing’s existing Library identity when that uniqueId is already distinct from the author’s). Overlay start URLs / uniqueId come from the clone. Fail closed if clone fails or reuses the author’s uniqueId. Author overwrite that still shares the author’s uniqueId (old path) mints a new Library uniqueId. Does **not** run a purge of the author’s uniqueId. Does **not** run on **Retire**, **Rename**, or a category move.
...
1. **Publish / overwrite from a My Tawala project** — the same `TawalaTransfer.publishToLibraryAfterStripConfirm` call as the My Tawala Publish dialog (source project + name + optional overwrite target), run from one place for every project instead of opening each one's Details page. Duplicate Library product is refused before the empty Library clone (same author/update rule as Details). Library copy is empty; My Tawala data is unchanged. No keep-data checkbox — see § Publish above.
...
3. **Retire** — remove *any* Library id from the public listing with **no replacement**, always keeping a My Tawala safety copy (`TawalaTransfer.retireLibraryEntry`; never hard-deleted):
   ...
   - **Leftover overlay (`retire1`):** listing skips overlay when `libraryRetired[id]` is set (a retired Publish like House Test must not reappear). A second Retire is idempotent — still removes leftover overlay, no “Already retired.” error, no second My Tawala copy. Successful Publish `clearLibraryRetired` so that slug can list again.
...
**Smoke:** `library-admin.html` with admin mode off → gate screen only, no table. Enable → table lists every current Library entry (stub + non-stub, with “stub” / “live” / “not vetted” / “published overlay” badges) plus a Publish/overwrite mini-form and a Retired section (empty until something is retired). Publish a Copy of catalog OEB with overwrite = None → refuse duplicate (no overlay). Publish/overwrite an original that has saved responses → Library copy empty, My Tawala Records unchanged. There is no keep-data checkbox. Publish a My Tawala row with **no live form** (no uniqueId, no `/p/{id}/` on starts) → refused, no overlay. Rename a row → name updates on `library.html` after reload, **no purge call** (Rename never purges). Retire a stub → gone from `library.html`, appears in `mytawala.html` still named `"… (stub)"`, **no purge call** (Retire never purges). Retire a non-stub (e.g. a Main Menu template) → gone from `library.html`, appears in `mytawala.html` under its original name. Retire a **live uniqueId** row → `:3001` must be up; if Tomcat has no `retireDeployment` yet, Retire **aborts** (listing stays) with a rebuild-WAR note. After a successful live Retire, occupancy no longer blocks that **name**. Restore either → back in `library.html` (Tomcat name stays vacated). Review: `http://localhost:5500/library-admin.html?v=20260826-pubpriv1&admin=1`. House Test retired overlay: hard-refresh `library.html?v=20260826-pubpriv1` — gone; second Retire is idempotent hide.
...
On **Project Details** (and My Tawala listing bar when a row is selected), **Make a Copy** forks the author’s **own** project without opening Designer: mint a **new overlay id**, **Push** a **new** Tomcat uniqueId, copy **all** source submissions onto that id, and let the author **rename** (for example “Biology Mid-term 2026”). **Rename** alone is **not** a new instance — only Make a Copy (or Library Copy to MyTawala) creates a new project identity. Original row stays untouched. Use / Deploy / Publish work on the copy immediately. Owner **Purges** for a clean slate.

**Wired Aug 10 (Task #9); clone-on-copy Aug 26; full data copy Aug 26:** `TawalaTransfer.makeCopyOfMyTawalaProject` + dialog (overwrite warn). Same `/api/deploy` mint path as Copy to MyTawala (`_freshFromTemplate`). Needs a definition to Push (clone snapshot, `jsonFile`, or live uniqueId). Fail closed if `:3001`/`:8080` cannot clone — **no** uniqueId-null leftover row. Does **not** copy source uniqueId or Versions history. After mint, export/import **all** submissions from the source uniqueId (no form-name filter). Fail-open if that data copy fails — clone stays; alert that data did not copy. Older forks that shared a uniqueId are still scrubbed on load (`scrubSharedForkRuntimes`); private clones with their own uniqueId are left alone. Lands on the new fork’s **Project Details** (`?forked=1`). Review: `http://localhost:5500/mytawala.html?v=20260826-pubstrip1` / Details `?v=20260826-pubstrip1`.

On the Library side, **Save a copy** acquires into My Tawala and should support **rename on acquire** the same way.

After the author **owns** a private copy (Library **Copy to MyTawala**, or Details / listing **Make a Copy**), the next commands are **Use / Deploy / Publish** — see the subsection below. Make a Copy does not distribute to participants or put the project in the public Library.
...
**Project Details action bar:** **RENAME · MAKE A COPY · BACKUP · RESTORE · DEPLOY · PUBLISH**. Residual confusion elsewhere: Versions still has **Push this version** (Designer-Push family). Listing-sidebar **Push from Designer** chrome **retired Aug 25 (#18)**. Make a Copy / Copy to MyTawala Push a private live form (Use ready). Leftover uniqueId-null rows still say **Edit in Designer → Push**. Backup/Restore stay distinct from both Deploy and Push (see glossary).

**Cross-links:** Task [#9](#task-list-aug-9) Make a Copy = own fork (new identity + private live uniqueId) — **wired Aug 10; clone-on-copy Aug 26**. Task [#10](#task-list-aug-9) = distribute + embed start-point work that website **Deploy** leads into. **Use** ≠ **Deploy** (website) ≠ **Push** (Designer) ≠ **Publish** (see glossary § [Save / Push / Deploy / Publish](#save--push--deploy--publish-product-glossary)).
...
5. **Use works after Copy to MyTawala / Make a Copy** — both Push a private live clone. **Copy to MyTawala** is an **empty** clone (no catalog submissions). **Make a Copy** (own fork) then copies **all** source submissions onto the new uniqueId. Listing **Use** opens the **new** `:8080` uniqueId. Original / Library Test Drive uniqueId unchanged. Edit in Designer on a Library acquire must open the clone snapshot (`deployIdentityName`), never the catalog JSON named for the public Library.
...
**Test Drive** belongs on the **Library** for ordinary try-outs (surveys, Horses, Get Together). When the visitor leaves the Test Drive, **purge** the demo responses. **Data-driven / derivative** apps (Online Exam Builder, Mongolia-style published exams — questions live in submissions) are **used from My Tawala**, not Library Test Drive: listing Test Drive is disabled; **Copy to MyTawala** stays.
...
**What Copy copies:** `TawalaDemo.libraryTestDriveUrl(project)` — e.g. Simple Survey `http://localhost:8080/p/gy1zssbrwm4fgfm/npwtqlg.Survey`. Data-driven exam rows do not copy a Test Drive link (control disabled).

**Smoke Test Drive honesty (#14 — Aug 24 lib1; exam off Aug 26; acquire empty `pubstrip1`):** open `http://localhost:5500/library.html?v=20260826-pubstrip1` (logged-out is fine).

1. Listing hint under the title reads wipe-on-**start**, not leave; Copy link = shared uniqueId; exam apps used from My Tawala.
...
9. **Make a Copy (own project)** — **DONE (Aug 10; empty-runtime fix same day; clone-on-copy Aug 26; full data copy Aug 26):** Project Details **MAKE A COPY** + My Tawala listing bar (selection-gated). Fork with new overlay id, rename dialog + overwrite warn, original untouched, **new** Tomcat uniqueId, **all** source submissions copied onto that id. Use / Deploy / Publish work without opening Designer. Fail closed if `:3001`/`:8080` cannot clone (no uniqueId-null leftover). Fail-open if data copy fails (clone stays; alert). Owner Purges for a clean slate. ≠ Rename ≠ Library Copy to MyTawala (empty clone). Review: `http://localhost:5500/mytawala.html?v=20260826-pubstrip1`.
...
14. **Test Drive contract** — **PARTIAL (honesty slice Aug 24 lib1):** Keep Library-owned; allow all start points during the drive. **Mock still purges on start** (not on leave) — a static `:5500` page cannot see the `:8080` tab close; do not fake leave-detection. **Honesty copy shipped:** Library listing hint + Test Drive / Copy link tooltips + Copy-link alert say wipe-on-start, shared Library uniqueId, closing the tab does not wipe. **Sep 8 — send-mail apps (List Builder):** listing honesty `*Email` keys + **Sends real email.** badge (`isSendsRealEmailProject`); in-form `TWarn` removed so My Tawala Use is not confused. **Version 2:** runtime Test Drive flag so a form item can show only during TD (`DESIGNER_OPEN_TODOS.md` **B9**). **Near-term done (Aug 10):** Test Drive works **logged-out**; Library **Copy Test Drive link**. ≠ My Tawala Deploy Copy link. **Still open:** real leave/wipe + per-drive uniqueId when Library goes public — see § [When to leave the mock for real :8080 Test Drive sessions](#when-to-leave-the-mock-for-real-8080-test-drive-sessions). End-page promo stays HOLD [#23](#task-list-aug-9).
...
18. **Library stub cleanup** — **DONE (Aug 25 #18):** Catalog seed has **0** WebLibrary stubs (Aug 10). Website path into Library is Details **PUBLISH** (wired Aug 1). **Publish refuses a My Tawala row with no live form** (Aug 26) — no overlay, owner copy *Edit in Designer, Push, then Publish.* Retired leftover My Tawala listing **Push from Designer** chrome (opened blank `:5173` as if that were the site go-live hop). Authors still **Push** from Designer (**Show in My Tawala**); the site hop is Details **Edit project in Designer**. Copy to MyTawala and Make a Copy already clone a live form (#26 / #9 Aug 26). Leftover uniqueId-null rows still say Edit in Designer → Push. See § Library stubs. Review: `http://localhost:5500/mytawala.html?v=20260825-stub18` and Details `?v=20260826-makecopy1`.
...
26. **P0 WIRED (Aug 24) — Private uniqueId on Copy to MyTawala / Get from Library** — Copy to MyTawala **Pushes** the catalog JSON under a minted Tomcat name (`_freshFromTemplate`) so the acquire gets a **new** uniqueId. **Aug 26:** empty clone only — does **not** import submissions from the catalog/author uniqueId (`pubstrip1`). `mockSharedLibraryRuntime` is no longer set on new copies. Old shared-id overlay rows are **scrubbed** on My Tawala load (Use greys until they Copy again). Edit in Designer opens the clone **snapshot** (with `deployIdentityName`) — never the catalog template JSON (that Push would Redeploy public Simple Survey). Fail closed if `:3001` / `:8080` cannot clone (no My Tawala row). **Out of scope:** Test Drive per-drive uniqueId ([#14](#task-list-aug-9)). Smoke: Copy Simple Survey → Use URL uniqueId ≠ `gy1zssbrwm4fgfm`; Records start empty; Library Test Drive still stock; Edit in Designer status shows the minted identity. **Owner smoke Aug 24:** Smoke26 Puddle on the copy only; Library still Pacific Ocean.
...
**Online Exam name vs id (cleaned Aug 26):** Public live id is `u3hkqgwtrepjlur` (the owner-checked 8-3-26 copy). Tomcat name is **Online Exam Builder**. Sibling `455sem0swhcswu5` is retired (`Online Exam Builder (retired 455sem0swhcswu5)`). Catalog URLs were already `/p/u3hk…/` and were not retargeted. Occupancy on the bare title now protects that same public uniqueId. Extra OEB clones retired: `-8-3-27`, `Builder1`, `Builder2`, Japan Exam, House Test Tomcat rows.

**Known mock debt (Task #26 — owner Aug 24):** Library **Copy to MyTawala** and My Tawala **Make a Copy** Push a private clone (new uniqueId). Pre-#26 overlay rows that still had `mockSharedLibraryRuntime` are scrubbed on load. Pre-clone Make a Copy forks that leaked a source uniqueId are still scrubbed; private clones with their own uniqueId are left alone. Do not treat a scrubbed acquire as a broken Library catalog.
