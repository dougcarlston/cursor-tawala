...
- **Get / Refresh** (My Tawala listing bar, Aug 9 option 3) — **Get from Library…** always on → `library.html` (Save a copy acquire). **Refresh from Library** only when a row is selected **and** linked to a Library entry (explicit `pulledFromLibraryId` or exact Library twin); runs the existing Pull metadata refresh. **Delete** still selection-gated.
...
**GET FROM LIBRARY…** (Aug 9 option 3): always-enabled listing control → `library.html` (public catalog / Save a copy acquire). Does **not** use the highlighted row.
...
**Smoke Purge:** with `:3001` + Postgres up — `curl -s -X POST http://localhost:3001/api/purge-responses -H 'Content-Type: application/json' -d '{"uniqueId":"gy1zssbrwm4fgfm","credentials":{"user":"dev","password":"dev"}}'`. UI: Deploy a project → **Show in My Tawala** → open Project Details → **PURGE** → confirm → alert shows deleted count; Test drive still purge-on-start. Undeployed seed → Purge → “no uniqueId / Deploy first” alert.
...
**Smoke Get / Refresh:** open http://127.0.0.1:5500/mytawala.html → **GET FROM LIBRARY…** (no selection needed) → lands on Library. Select **Online Exam Builder** (Library twin) → **REFRESH FROM LIBRARY** enables → dialog → **Refresh** → confirm → description/category refreshed; **name** / deploy identity unchanged. Select a private-only row (e.g. Paypal Tester) → Refresh stays disabled with a “not linked” tooltip; **DELETE** still enables.
...
3. **Records (Responses) count** — **DONE (Aug 9 slice):** project-wide total on My Tawala list + Details (`TawalaDemo.countResponses` → existing `:3001/api/export-responses`). Per-form counts in Project Data. Shows a real number when `uniqueId` + Docker/Postgres (or dev-session) are up; otherwise honest **"—"** (no fake zeros). No dedicated count endpoint yet — export is the path.
4. **Project Data section** — **DONE (tree + forms, Aug 9):** project **+** cycles closed → start points → all forms (second click; Forms **+** optional). Online Exam seeded `formNames` + `projects/mytawala/Online Exam Builder.json` hydrate. Selection scopes Export / Import / Purge (enablement matrix A/B/C). Whole-project Purge already on the Details action bar; **Selective (per-form) Purge** still next. **HOLD nuance:** sports “stats-only” Selective Purge after commissioner feedback if needed.
...
- **Records column alignment** in the Project Data banner (heads vs values vs form-row counts under the shared grid) was iterated in CSS/`project-ops.js` this afternoon but is **still being verified visually** — treat as open polish, not signed off. Counts themselves are wired (live number or honest **—**).
- Selective (per-form) Purge still next after whole-project Purge (Task List item 4).
