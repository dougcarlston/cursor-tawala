...
**Smoke Publish:** open http://localhost:5500/mytawala-project.html?v=20260826-pubuid1 (or Library admin `?v=20260826-pubuid1&admin=1`). **Refuse:** Publish the leftover **Horses** (or any row with no live form / no `/p/{id}/` starts) → Details dialog disables Publish (or admin click fails) with *This project has no live form yet. Edit in Designer, Push, then Publish.* — no new Library overlay. **Still works:** Publish a Designer-**Push** row that has a live form → overlay writes as before. Then: open a live project → Project Details → **PUBLISH** → dialog opens with the name pre-filled (no keep-data checkbox) → type a new name, e.g. “DirtBowl League Manager”, pick the flagged “DirtBowl (stub) — retires to My Tawala, kept marked (stub)” target → **Publish** → confirm → if Records > 0, extra confirm *Project will be stripped of data upon publication. Proceed?* (Cancel leaves the project unpublished and unpurged) → alert confirms the new Library entry, the retired stub, and a stripped-row count when data was stripped → reload `library.html`: new name appears (no `(stub)` suffix), old “DirtBowl (stub)” row is gone → reload `mytawala.html`: a new private row named “DirtBowl (stub)” appears (safety copy). Repeat picking **None** to just add a new entry with no retirement, and picking a **non-stub** target (e.g. an out-of-date Main Menu template) to see it replaced in place with no My Tawala copy. Empty templates skip the strip confirm. Pick a project with a live `:8080` deploy and prior submissions, Publish it (accept strip), then **Copy to MyTawala** the newly published Library entry — Records on the acquire should start empty.
...
**Scratch (Aug 26):** same-uniqueId partial purge (keep Question/SetupVariables) **reverted** — it scrambled Online Exam. Data-driven Library entries have **no Test Drive**. **Copy to MyTawala** = empty clone (does not import catalog submissions). **Make a Copy** (own fork) copies **all** response data onto the new uniqueId. **Publish** with Records > 0: confirm *Project will be stripped of data upon publication. Proceed?* then whole-uniqueId strip, then overlay. Empty templates skip the extra confirm. Review `library.html?v=20260826-pubstrip1`.
...
**Strip data on Publish (owner Aug 26, 2026):** cannot safely Publish a product **with data**. If the source has any submissions (same uniqueId count as Purge / Records): extra confirm *Project will be stripped of data upon publication. Proceed?* Cancel = do not publish and do not purge. Proceed = whole-uniqueId `purgeAfterPublish` (same as the old Purge-on-publish checked path), **then** write the Library overlay. Empty templates (count 0) skip this extra confirm and publish as today. There is **no** checkbox to keep data on Publish. If strip fails, overlay is **not** written.
...
3. **Confirm** — a `window.confirm()` spelling out the consequence (new entry / stub retirement / in-place replace). If Records > 0, a **second** confirm is the exact strip string above.
4. **Strip then overlay** — when there is data and they proceed, purge first; only then write `tawala.mock.libraryOverlay`.

**Purge-on-Publish (updated Aug 26, 2026):** strip runs **before** the overlay write when count > 0 (after the strip confirm). Same uniqueId resolution as My Tawala **PURGE** (`resolvePurgeUniqueId`), same `POST :3001/api/purge-responses` call. If strip fails, Publish **aborts** (no Library overlay). Empty templates skip strip. Does **not** run on **Retire**, **Rename**, or a category move.
...
1. **Publish / overwrite from a My Tawala project** — the same `TawalaTransfer.publishToLibraryAfterStripConfirm` call as the My Tawala Publish dialog (source project + name + optional overwrite target), run from one place for every project instead of opening each one's Details page. If the source has responses: exact strip confirm, then purge, then overlay. Empty templates skip the extra confirm. No keep-data checkbox — see § Publish above.
