...
**Mock today:** `.backup.json` already *tries* to include definition + data + properties. Restore reapplies properties + submissions only — it does **not** Push the definition back to `:8080` / Designer. Next build (when we expand Backup): Restore must Redeploy the bundled definition onto this uniqueId, then replace responses. Format may stay JSON in the mock; live Java remains a `.backup` ZIP.
...
10. **Start points: distribute + embed** — **PARTIAL (Aug 10 doorway; Aug 25 picker + copy polish):** Details **DEPLOY** (and Invite / Include) open the share panel. Invite focuses the link; Include focuses the embed. Project Data start highlight preselects that start. One-start projects skip the dropdown. **Owner Aug 25:** share-panel language clearer (Share a live form). **HOLD:** uniqueId-in-URL hardening; owner-chosen participant/admin start labels. See § [Use / Deploy / Publish](#use--deploy--publish-after-owning-a-project).
...
15. **DECIDED (Aug 25) — Backup package** — Backup = live pushed definition + current responses + My Tawala properties (now, not version history). **Restore** = time machine onto this same uniqueId (definition then data). **Push this version** = switch live definition only. Mock Restore still does not Redeploy the bundled definition — next Backup build, not UI chrome. See § [Backup](#backup).
...
Continue the [Task List](#task-list-aug-9). **Next P0:** [#26](#task-list-aug-9) private uniqueId on Copy to MyTawala (stop sharing Library `:8080` ids). Task #14 **honesty copy** landed Aug 24; real leave/wipe + per-drive uniqueId still wait for public Library. Polish Task #10 as needed. Designer → **Push** rename stays on the Designer chat list. Do not re-debate Aug 9 list 1+4 decisions unless new evidence appears. Pre-live Auth / Payments / Email metering stay HOLD. End page / save-data auth / Library category IA ([#23](#task-list-aug-9)–[#25](#task-list-aug-9)) stay HOLD.
