// $Workfile: FieldUtil.cs $
// $Revision: 8 $	$Date: 2/27/08 4:01p $
...
namespace Tawala.Projects
{
	public class FieldUtil
	{
		...
		{
			return IsRecordField(fieldString) || IsExternalRecordField(fieldString) ? Regex.Match(fieldString, recordFieldPattern).Groups[1].Value : string.Empty;
		}
		...
		public static string UnknownFieldName
		{
			get { return "Unknown Field"; }
		}
		...
		/// </remarks>
		public static string DefaultRecordQualifierName
		{
			get { return "Record"; }
		}

		public static Record DefaultRecordQualifierRecord
		{
			get { return record; }
		}

		private static Record record = new Record(DefaultRecordQualifierName);
		...
		{
			IPaletteField paletteField = field as IPaletteField;
		...
		public static IField RecordQualifySharedDataField(IField field)
		{
			...
			{
				//NewBlank b = paletteField as NewBlank;
			...
			{
				IMcqItem mc = paletteField as IMcqItem;
				if (mc != null && mc.Form is IExternalForm)
				{
					return new RecordField(DefaultRecordQualifierRecord, paletteField);
				}
			}
		...
		private static IPaletteField resolveFormField(string fullyQualifiedFieldName)
		{
			...
			IPaletteField field = form.ItemList[fieldName] as IPaletteField;
