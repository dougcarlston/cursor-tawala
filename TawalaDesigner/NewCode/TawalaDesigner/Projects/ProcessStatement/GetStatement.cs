// $Workfile: GetStatement.cs $
// $Revision: 18 $	$Date: 8/23/07 12:24p $
...
{
	[Serializable]
	public class GetStatement : Tawala.Projects.ProcessStatement
	{
		public GetStatement()
		{
			name = "Get";
		...
		public GetStatement(RecordSet records)
			: this()
		{
		...
		private void constructFromXml(IXmlElement element, Process process)
		{
			...
			this.records = new RecordSet(element.GetAttribute("recordList"), forms);
		...
		{
			string resultString = Name + " " + records.FieldName + Properties.Resources.GetStatementFrom;
		...
		{
			return resultString += FieldProviders.CreateValidFormListString(records.Forms, Properties.Resources.GetStatementNoFormsSelected);
		}
		...
		private string appendConditions(string resultString)
		{
			...
			{
				resultString += Properties.Resources.GetStatementWhere;
