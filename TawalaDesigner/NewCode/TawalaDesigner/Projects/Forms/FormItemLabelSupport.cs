...
namespace Tawala.Projects.Forms
{
	public static class FormItemLabelSupport
	{
		...
		public static bool IsLegalItemLabel(string label, IFormItem targetFormItem)
		{
			return
				isLegalLabel(label) &&
				!ItemLabelDuplicatesOtherItemLabel(label, targetFormItem) &&
				!ItemLabelDuplicatesAnyBlankLabel(label, targetFormItem) &&
				!ItemLabelDuplicatesHiddenFieldName(label, targetFormItem);
		}
		...
		public static bool IsLegalBlankLabel(string label, IBlank targetBlank)
		{
			return
				isLegalLabel(label) &&
				!BlankLabelDuplicatesAnyItemLabel(label, targetBlank) &&
				!BlankLabelDuplicatesOtherBlankLabel(label, targetBlank) &&
				!BlankLabelDuplicatesHiddenFieldName(label, targetBlank);
		}

		private static bool isLegalLabel(string label)
		{
			return
				!resemblesDefaultFormItemLabel(label) &&
				!hasLeadingDoubleUnderscores(label) &&
				!containsColon(label) &&
				!isNumeric(label) &&
				containsValidLabelCharacters(label);
		}
		...
		private static bool labelDuplicatesHiddenFieldNameInForm(string label, IForm form)
		{
			if (form != null)
			{
				foreach (IFormItem item in form.ItemList)
				{
					IHiddenField hiddenField = item as IHiddenField;
		...
		private static IForm getContainingFormForBlank(IBlank targetBlank)
		{
			foreach (IForm form in Project.Current.FormList)
			{
				foreach (IFormItem item in form.ItemList)
				{
					IFibItem fibItem = item as NewFibItem;
