...
namespace Tawala.Projects
{
	...
	{
		ElementSignatureList elementSignatures = new ElementSignatureList();

		public int RegisteredEntries
		{
			get
			{
				return elementSignatures.Count;
			}
		}
		...
		public T MakeObject(IXmlElement element)
		{
			...
			if (factoryMethod == null)
			{
				...
				if (constructor == null)
				{
					Console.WriteLine("Factory.MakeObject: Cannot find constructor {0}(IXmlElement) for XML element <{1}>", classType.ToString(), element.Name);
				}
		...
		public T MakeObject(IXmlElement element, IField fieldResolver)
		{
			...
			if (constructor == null)
			{
				Console.WriteLine("Factory.MakeObject: Cannot find constructor {0}(IXmlElement, fieldResolver) for XML element <{1}>", classType.ToString(), element.Name);
			}
		...
		public T MakeObject(IXmlElement element, string processName)
		{
			...
			if (constructor == null)
			{
				Console.WriteLine("Factory.MakeObject: Cannot find constructor {0}(IXmlElement, string) for XML element <{1}>", classType.ToString(), element.Name);
			}
		...
		public T MakeObject(IXmlElement element, Process process)
		{
			...
			if (constructor == null)
			{
				Console.WriteLine("Factory.MakeObject: Cannot find constructor {0}(IXmlElement, Process) for XML element <{1}>", classType.ToString(), element.Name);
			}
	...
	/// </summary>
	class ElementSignature : IComparable
	{
		private string elementName;
		private StringCollection attributeNames = new StringCollection();
		...
		{
			ElementSignature other = obj as ElementSignature;
		...
		public int CompareTo(object obj)
		{
			...
			{
				ElementSignature otherSignature = obj as ElementSignature;

				if (this.elementName == otherSignature.elementName)
				{
					// signature with greatest number of attributes is the "lesser" signature
					return (otherSignature.attributeNames.Count - this.attributeNames.Count);
				}
				else
				{
					// element names compare alphabetically
					return (this.elementName.CompareTo(otherSignature.elementName));
				}
			}
	...
	/// </summary>
	class ElementSignatureList : ArrayList
	{
		...
		public void Set(ElementSignature signature)
		{
			...
			{
				ElementSignature s = (ElementSignature)this[i];
