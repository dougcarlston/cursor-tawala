...
namespace Tawala.FormDesigner
{
	public class FormPresenter : IFormPresenter
	{
		...
		private bool viewInitializationCompleted;
		...
		public bool CanPasteFormItems
		{
			get { return Clipboard.GetDataObject().GetDataPresent(typeof (IFormItem[])); }
		}
		...
		{
			View.InsertHeadingItem(headingItem.Contents.ToXhtml(headingItem), adjustInsertionIndex(index), headingItem.Id, headingItem.HeadingType.ToString());
		...
		{
			View.InsertTextItem(textItem.Contents.ToXhtml(textItem), adjustInsertionIndex(index), textItem.Id);
			updateAllLabelsInView();
		}

		private void insertFormItemIntoView(NewFibItem fibItem, int index)
		{
			View.InsertFibItem(fibItem.Contents.ToXhtml(fibItem), adjustInsertionIndex(index), fibItem.Id);
			updateAllLabelsInView();
		}
		...
		public void SynchronizeFormItems()
		{
			if (viewInitializationCompleted)
			{
		...
		{
			headingItem.Contents = FormItemContentsFactory.MakeChildrenFromHtml(View.GetHtmlElementById(headingItem.Id.ToString()).InnerHtml);
		}

		private void syncFormItemToModel(ITextItem textItem)
		{
			textItem.Contents = FormItemContentsFactory.MakeChildrenFromHtml(View.GetHtmlElementById(textItem.Id.ToString()).InnerHtml);
		}

		private void syncFormItemToModel(IMcqItem mcqItem)
		{
			mcqItem.Contents = FormItemContentsFactory.MakeChildrenFromHtml(View.GetAttribute(mcqItem.Id.ToString(), "Contents"));
		}

		private void syncFormItemToModel(NewFibItem fibItem)
		{
            fibItem.Contents = FormItemContentsFactory.MakeChildrenFromHtml(View.GetHtmlElementById(fibItem.Id.ToString()).InnerHtml);
		}
