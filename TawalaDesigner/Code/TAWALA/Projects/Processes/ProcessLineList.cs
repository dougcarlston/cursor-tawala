...
namespace Tawala.Projects.Processes
{
    ...
    public sealed class ProcessLineList : Collection<ProcessLine>
    {
        ...
        public ProcessLineList(ProcessStatement statement)
        {
            ...
            GetType().InvokeMember("createLineList", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.InvokeMethod, null, this,
...
// ReSharper disable UnusedMember.Local
        ...
        private void createLineList(IfStatement statement)
        {
            ...
            if (!statement.HasOtherwise)
            {
                ...
                tmp.Add(new IfBlockCloseLine(statement, ")", "</trueSet>\r\n</if>"));
            }
            else
            {
                // make closing parenthesis line
                ...
                // make opening parenthesis line
                tmp.Add(new IfBlockOpenLine(statement, "(", "<falseSet>"));
                ...
                // make closing parenthesis line
                tmp.Add(new IfBlockCloseLine(statement, ")", "</falseSet>\r\n</if>"));
            }
        ...
        private void createLineList(ForEachRecordStatement statement)
        {
            var tmp = new ProcessLineList
                      {
                          new ForEachRecordLine(statement),
                          new ForEachBlockOpenLine(statement, "(", "")
                      };
            ...
            // make closing parenthesis line
            tmp.Add(new ForEachBlockCloseLine(statement, ")", "</foreach>"));
...
// ReSharper restore UnusedMember.Local
        ...
        private void replaceIfLines(int index, IfStatement statement)
        {
            ...
            var oldStatement = (IfStatement)oldLine.Statement;
            ...
            ProcessLine newLine = new IfLine(statement);
            this[index] = newLine;
        ...
        {
            var tmpList = new ProcessLineList();
        ...
        {
            var copiedList = new ProcessLineList();

            var groupIDs = new Hashtable();
        ...
        public void DragDrop(int sourceIndex, int targetIndex)
        {
            ...
            if (sourceGroup != null && sourceStatement != null)
            {
                ...
                var sourceList = new ProcessLineList(sourceIndex, this);
        ...
        {
			for (int i = lastLineIndex; i >= firstLineIndex; i--)
			{
        ...
        private int enclosingForEachBlockOpenLineIndex(int processLineIndex)
        {
            ...
            do
            {
                ...
                if (i >= 0 && i < Count)
                {
					if (this[i] is ForEachBlockCloseLine)
                    {
                        outdentLevel--;
                    }
                    else if (this[i] is ForEachBlockOpenLine)
                    {
                        outdentLevel++;
                    }
        ...
        /// <param name="processLineIndex"></param>
        /// <returns>-1 if process line index not enclosed by ForEachLine</returns>
        private int enclosingForEachBlockCloseLineIndex(int processLineIndex)
        {
            int index = -1;
            Boolean found = false;
            int parenCount = 0;
            int i = enclosingForEachBlockOpenLineIndex(processLineIndex);

            do
            {
                if (i >= 0 && i < Count)
                {
                    if (this[i] is ForEachBlockOpenLine)
                    {
                        parenCount++;
                    }
                    else if (this[i] is ForEachBlockCloseLine)
                    {
                        parenCount--;
                    }

                    if (parenCount == 0 && this[i] is ForEachBlockCloseLine)
                    {
                        found = true;
                        index = i;
                    }
        ...
        public ProcessStatement GetEnclosingForEachStatement(int processLineIndex)
        {
            ...
            int firstIndex = enclosingForEachBlockOpenLineIndex(processLineIndex);
            int lastIndex = enclosingForEachBlockCloseLineIndex(processLineIndex);
        ...
        {
            var list = new ProcessStatementList();
        ...
        protected override void InsertItem(int index, ProcessLine item)
        {
            ...
            if (item.Statement != null && raiseEvent())
            {
        ...
        protected override void RemoveItem(int index)
        {
            ...
            if (line.Statement != null && raiseEvent() && !updating)
            {
                // raise "statement removed" event
                ...
                else
                {
                    ...
                    if (Project.Current.CurrentComponent is IForm)
                    {
                        ...
                        foreach (IFormItem formItem in form.ItemList)
                        {
                            ...
                            {
                                var skip = formItem as ISkipInstructionsItem;
