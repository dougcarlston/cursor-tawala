...
- **Process script — opening quote must not orphan at line end (owner Sep 4)** — Readability / typography, not a product bug. When a Process (or Skip) script line wraps, an opening `"` must stay with the following word/token (treat `"` as part of that word). Bad: `equals "` / next line `<<DeleteRecord:NameToRemove>>"`. Good: wrap before the `"` so `"<<DeleteRecord:NameToRemove>>"` stays together. Shot: `assets/Polish_-_Process-script-opening-quote-line-wrap-Sep4.png`. Fix later in script-line render (e.g. nowrap around `"…"` runs or word-joiner after `"`). Polish when touching Process script display — not Library/travel blocking.
...
| **B5** Project Manager + Email Delivery menu | **Project Manager** = real (My Tawala / site). **Email Delivery** menu item: **hide stub** — leave delivery as-is; **Library-side** email metering tracks cost per registered user (My Tawala group senders). Most Test Drive volume is negligible; **exception: Automated List Builder** sends real solicitation/admin mail — V1 warning is **Library Test Drive honesty** (listing tooltip / Copy-link alert / row badge), not in-form copy (My Tawala must not see Test Drive language). Pull from Library TD if that is not enough |
| **B9** Test Drive runtime flag for in-form warnings | **Version 2** — Library Test Drive URLs (or session) expose a flag form items can condition on, so a warning like List Builder `TWarn` can show during TD and stay hidden in My Tawala Use. V1 (Sep 8) = listing honesty only (`tooltipSingleEmail` / `isSendsRealEmailProject`). Do not fake this with a query param the Java runtime ignores |
