...
- **Project → Page Header… image editor** — **Reinstall after trip (owner Aug 10).** In-dialog pan/crop/resize was mothballed (Jul 28) because baking to banner size looked softer than Deploy’s full-res + `object-fit: cover`. Owner now wants that graphics tool **back**, and is open to **removing the non-distort / aspect-lock** so authors can stretch banners sideways (long headers vs square source photos). Spec: `DESIGNER_PAGE_HEADER.md`. Keep Deploy sharp (prefer crop/viewport over downsampling when possible).
...
| 17 | **Page Header image editor (reinstall)** | **Owner Aug 10:** Restore mothballed in-dialog pan/crop/resize; allow sideways stretch (drop non-distort). Keep Deploy sharp. Spec: `DESIGNER_PAGE_HEADER.md`. |
| 18 | **New Project / uniqueId data isolation** | **Owner Aug 20:** New Project must never inherit old `:8080` submissions; distinct uniqueIds must not paint each other’s blank values. Detail: `DESIGNER_OPEN_BUGS.md` § Aug 20. |
