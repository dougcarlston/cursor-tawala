...
- **Cannot rename Form / Process / Document in Explorer** — **Fixed Jul 19; policy updated Sep 2026.** Jul 19: F2 + click-selected-to-rename (long-press fallback). **Owner Sep 2026:** single-click on an already-highlighted row was entering rename and blocking open-to-canvas. **Now:** single-click always opens/focuses the MDI window; **double-click** or **F2** renames. Long-press rename removed. Escape cancels; Enter commits.

- **Cannot rename Process in Explorer (Forms/Documents OK)** — **Fixed Jul 20:** a linked process appears twice (under the form and under **Processes**). Inline rename matched `kind + name`, so both rows entered edit mode; the second input stole focus and blur-cancelled the first. Now matches the unique render `key` so only one `RenameInput` mounts. **Smoke:** select a Post/Pre (or Processes-folder leaf) → **F2** or **double-click** → type new name → Enter; Explorer + open window title update.
...
- **Process insert / IF panel dead after New Document (or Explorer click) while Process window stays open** — **Fixed Sep 2026.** Creating a Document (Explorer toolbar) changed `selection` to that document but left the Process MDI window as `activeWindowId`. Insert gaps, IF row clicks, and Statements palette early-returned on `selection.kind !== "process"` — owner had to click away then back to regain control. Fix: resolve Process target from **active MDI Process window first**, re-sync Explorer selection on Process interactions, and re-call `focusWindow` on pointer-down when selection drifted. Smoke: open Post-Administration → select IF → New Document → click IF again (properties return) → set insert gap inside then → Statements → Show Document inserts without click-away. Tests: `projectStore.processFocus.test.ts`.
