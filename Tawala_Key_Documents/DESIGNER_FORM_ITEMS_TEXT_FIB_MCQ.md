...
6. Deploy: B/I/U (and face/size/color when set) on FIB prompt labels in Design must appear on the Java form after Redeploy — **including `topLabels` (MQS Name:/Age:) and left/right-align**, not only freeform (`fibRichPromptToXml` / `fibToXml`). Plain unstyled FIB labels stay regular weight (legacy); MCQ plain questions still auto-bold via `mcToXml`.
...
11. **Insert → Date / Address (parked Sep 2026):** Menu entries **hidden** until presets align with leftAlign label-column geometry in Design, Preview, and Push. Preset helpers remain in `fibInsertPresets.ts` / Sign-up template JSON. When re-enabled: Date → freeform `__/__/____`; Address → DirtBowl-style Street/City/Zip line or one-blank-per-row leftAlign rows.
...
| Insert → Date / Address | — (author builds by hand) | **Parked (Sep 2026)** — presets exist; Insert menu hidden until layout parity; smoke #11 |
