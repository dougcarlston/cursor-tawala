...
- **File → Save As…** / **⇧⌘S** / **Shift+Ctrl+S** opens an **in-app name dialog** (default = `{project.name}.json`, ensures a single `.json` suffix). **Cancel** aborts. **Save As** then:
  - **Chromium (`showSaveFilePicker`):** clears any remembered handle and opens the native folder/file picker with `suggestedName` = the chosen name; that location becomes the quiet re-Save target.
