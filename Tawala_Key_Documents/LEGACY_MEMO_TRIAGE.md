...
- **Owner smoke Aug 7, 2026 — Versions / Deploy-switch (updated after wire):** Early gap was radios that looked like Deploy-switch but did nothing. **Resolved same day:** **Deploy this version** wired for snapshot-backed rows (Deploy → Show in My Tawala saves definition snapshots). Delete-version still grey. **Restore ≠ make earlier definition version current** — mock Backup/Restore stays **data-oriented**; use Deploy this version for definition switch. **Import** field-mismatch on newer schema = **correct**. **Version Download** = minimal metadata JSON only. Honesty table: `website-mock/README.md` glossary **Owner smoke Aug 7**. **Page-feature triage (Aug 7–8):** canonical lists in `website-mock/README.md` § **Aug 7–8 legacy page triage** (stills under `website-mock/legacy-reference/stills/`).
- **Open product question (park Aug 7, 2026):** **Selective Purge by form** — e.g. SportsDashboards keep Player/Coach, purge statistics for a new year. Owner will ask commissioners if needed / if legacy supported it. Under **Purge / E/I/B/R**; do **not** implement. Also in `website-mock/README.md` Open questions + Parked / backlog.
...
- **Aug 7–8 legacy page triage (canonical):** `website-mock/README.md` § Aug 7–8 — My Tawala / Details / Library feature lists + stills in `website-mock/legacy-reference/stills/`
